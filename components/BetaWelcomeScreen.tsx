...
const ALL_TOOLS: Record<string, { title: string; icon: string; description: string; category: string }> = {
    ...
    google_analytics_playground: { title: 'Google Analytics Playground', icon: '📊', description: 'View traffic by medium, landing pages, and device performance.', category: 'Workflows' },
    merchant_center_playground: { title: 'Merchant Center Playground', icon: '🛒', description: 'View product feed health, performance metrics, and manage promotions.', category: 'Workflows' },
...
const BetaWelcomeScreen: React.FC<BetaWelcomeScreenProps> = ({ onSelectTool, allPerformanceData = [], allAccountDetails = [], userName }) => {
    ...
    const METRICS_COLS = { lgCols: 3, mdCols: 2, smCols: 1 };
    ...
    const [visibleLeaderboards, setVisibleLeaderboards] = useState<string[]>(() => {
        try {
            const saved = localStorage.getItem('beta-welcome-leaderboards-visible-v3');
    ...
    const [visibleNotableMetrics, setVisibleNotableMetrics] = useState<string[]>(() => {
        try {
            const saved = localStorage.getItem('beta-welcome-notable-metrics-v2');
    ...
    const LEADERBOARD_LAYOUT_KEY = 'beta-welcome-screen-leaderboard-layout-v22';
    const NOTABLE_LAYOUT_KEY = 'beta-welcome-screen-notable-layout-v23';
    ...
    useEffect(() => { localStorage.setItem('beta-welcome-notable-metrics-v2', JSON.stringify(visibleNotableMetrics)); }, [visibleNotableMetrics]);
    useEffect(() => { localStorage.setItem('beta-welcome-leaderboards-visible-v3', JSON.stringify(visibleLeaderboards)); }, [visibleLeaderboards]);
    ...
    }, [visibleLeaderboards, visibleNotableMetrics]);
